    ...
    if(this.state.fitnessCenters){
      return (
       ...
       <Icon raised reverse name='add' color='#E62221'
            ...
            onPress={() => navigate('SuperAdminAddGym')}
          />
        <SearchBar
          round
          lightTheme
          ...
          onChangeText={(gymInput) => this.setState({gymInput})}
          placeholder='Search fitness center...' />
          <ScrollView>
          {
            this.state.fitnessCenters
            .filter(i => this.state.gymInput === '' || i.name.includes(this.state.gymInput))
            .map((item, i) => (
              <ListItem
                key={i}
                ...
                onPress={() => navigate('SuperAdminViewGym', {fitnessCenter: item})}
              />
            ))
          }
          </ScrollView>
